...
export const format = {
  ...
  initialsName: (value: string) => {
    const name = value.toLowerCase();

    const words = name.split(' ');

    const complete = words.map(
  ...
  initialsNameLatter: (value: string) => {
    const name = value.toLowerCase();

    const words = name.split(' ');

    const complete = `${words[0].charAt(0).toUpperCase()}${words[words.length - 1] ? words[words.length - 1].charAt(0).toUpperCase() : ''}`;
  ...
  obscureEmail: (rawValue: string, length = 6) => {
    const [name, domain] = rawValue.split('@');
    return `${name.substring(0, length)}${new Array(7).join('*')}@${domain}`;
  },

  obscureEmail3Caracteres: (rawValue: string) => {
    const [name, domain] = rawValue.split('@');
    return `${name.substring(0, 4)}${new Array(7).join('*')}@${domain}`;
  },
...
export function formatNumber(num: number, precision = 1) {
  ...
  const newNum = Math.trunc(num / 100);

  const found = map.find((x) => Math.abs(newNum) >= x.threshold);
  if (found) {
    const formatted =
      (newNum / found.threshold).toFixed(precision) + found.suffix;
    return formatted;
  }
